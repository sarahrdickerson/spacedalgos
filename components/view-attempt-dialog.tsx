    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-[calc(100vw-2rem)] max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4">
            ...
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-3">
              <div>
                ...
                <div className="font-medium text-sm sm:text-base">
                  {formatDate(attemptDate)}
                </div>
              </div>
              <div className="sm:text-right">
                <div className="text-sm text-muted-foreground">Grade</div>
                <div
                  className={`font-semibold text-sm sm:text-base ${gradeColors[grade]}`}
                >
                  {gradeText[grade]}
                </div>
              </div>
          ...
          <Button
            variant="outline"
            className="w-full whitespace-normal text-sm h-auto py-2"
          ...
          {expanded && (
            <div className="space-y-3">
              {error && (
                <div className="text-sm text-destructive p-3 rounded-lg bg-destructive/10 break-words">
                  {error}
                </div>
              )}
                ...
                <div className="space-y-2">
                  ...
                  {history.attempts.map((attempt, index) => (
                    ...
                    >
                      ...
                      {attempt.note && (
                        <div className="text-xs text-muted-foreground bg-muted/30 p-2 rounded break-words">
                          {attempt.note}
                        </div>
                      )}
                    </div>
